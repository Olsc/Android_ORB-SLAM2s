...
- **Нативный AR-рендеринг**: 3D-рендеринг на основе **Google Filament** (модели GLB/glTF) и OpenGL ES, поддерживающий размещение и взаимодействие с виртуальными объектами на обнаруженных плоскостях.
...
- **Управление AR-объектами**: Поддержка размещения, масштабирования (жест сведения пальцев) и взаимодействия с 3D-объектами.
- **3DOF отслеживание ориентации**: Отслеживание ориентации с тремя степенями свободы с использованием встроенных датчиков устройства (Rotation Vector / Акселерометр + Магнитометр).
- **Web удаленный просмотр**: Встроенный SSL-шифрованный HTTP Web-сервер (HTTPS) для удаленного просмотра видеопотока с камеры и SLAM-данных через браузер.
- **Поддержка нескольких карт**: Одновременная загрузка, сопоставление и управление несколькими файлами карт.
...
- **[OpenCV 5 (Основной код)](https://github.com/opencv/opencv)**: Лицензировано под **Apache 2.0 License**. Мы извлекли только необходимый основной код, чтобы проект оставался легким.
- **[srrg_hbst](https://gitlab.com/srrg-software/srrg_hbst)**: Лицензировано под **BSD 3-Clause License**. Используется для быстрого и масштабируемого сопоставления изображений.
- **[DBoW2](https://github.com/dorian3d/DBoW2)**: Лицензировано под **BSD License**. Используется для базовых векторов словаря и структур сопоставления признаков.
- **[g2o](https://github.com/RainerKuemmerle/g2o)**: Лицензировано под **BSD License** (основные компоненты). Используется для нелинейной оптимизации.
- **[Eigen3](http://eigen.tuxfamily.org/)**: Лицензировано под **MPL2 (Mozilla Public License v2.0)**. Используется для матричных операций и алгебраических вычислений.
...
Мы используем [OpenCV](http://opencv.org) для манипуляции изображениями и признаками. **Обновлено до OpenCV 5**. Примечание: чтобы проект оставался легким, мы извлекли только необходимый код из OpenCV 5, а не импортировали всю библиотеку целиком.
