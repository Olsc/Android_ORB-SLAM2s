...
- **네이티브 AR 렌더링**: **Google Filament**(GLB/glTF 모델) 및 OpenGL ES 기반 3D 렌더링, 감지된 평면 위에 가상 객체 배치 및 상호 작용 지원.
- **어두운 프레임 감지**: SLAM 스레드 블로킹을 방지하기 위해 자동으로 어둡거나 저품질 프레임을 건너뜁니다.
- **AR 오브젝트 관리**: 3D 객체 배치, 확대/축소(핀치 제스처) 및 상호 작용 지원.
- **3DOF 방향 추적**: 내장 기기 센서(회전 벡터 / 가속도계 + 자기장 센서)를 사용한 3자유도 방향 추적.
- **Web 원격 보기**: 내장 SSL 암호화 HTTP 웹 서버(HTTPS)를 통해 브라우저로 카메라 영상 및 SLAM 데이터를 원격으로 확인.
