...
3. **权限最小化**：仅申请 SLAM 和 AR 功能所必需的最少权限，不申请任何多余权限。
4. **无后台服务**：应用无常驻后台服务，退出时释放所有资源（摄像头、传感器、GL 上下文）。
5. **应用备份与数据泄漏防范**：应用的 `AndroidManifest.xml` 声明了 `android:allowBackup="true"` 以提供便利。在 Android 12+（API 31+）上，用户可在设备设置中禁用备份，以防止地图文件和应用数据被包含在系统备份中。处理敏感地图数据的用户应考虑禁用应用备份功能。
6. **原生代码崩溃处理**：应用的 C++ 原生层包含信号处理框架，可拦截原生崩溃（如 SIGSEGV、SIGABRT）并生成诊断日志。这些日志仅写入 Android Logcat 缓冲区（仅在开发者/调试模式下可访问），应用本身从不收集、存储或传输这些日志。此机制仅用于开发过程中的调试目的。
