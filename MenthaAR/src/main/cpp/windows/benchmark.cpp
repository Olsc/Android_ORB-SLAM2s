...
#ifdef _WIN32
    // Windows: 使用 _popen/_pclose 与 where 命令探测 ffmpeg
    FILE* which = _popen("where ffmpeg 2>NUL", "r");
#else
...
#ifdef _WIN32
    const char* devNullRedirect = "2>NUL";
#else
    const char* devNullRedirect = "2>/dev/null";
#endif
    std::string cmd = "ffmpeg -y -i \"" + path + "\" -vcodec copy -an \"" + fixed + "\" " + devNullRedirect;
...
MemoryInfo getMemoryInfo(int numMPs, int numKFs) {
    MemoryInfo info;
#ifdef _WIN32
    // Windows: 通过 PSAPI 读取进程内存（工作集≈RSS，私有提交字节≈VMS）
    PROCESS_MEMORY_COUNTERS_EX pmc = {};
    pmc.cb = sizeof(pmc);
    if (GetProcessMemoryInfo(GetCurrentProcess(),
                             reinterpret_cast<PROCESS_MEMORY_COUNTERS*>(&pmc), sizeof(pmc))) {
        info.rssMB = pmc.WorkingSetSize / (1024.0 * 1024.0);
        info.vmsMB = pmc.PrivateUsage / (1024.0 * 1024.0);
        info.peakRssMB = pmc.PeakWorkingSetSize / (1024.0 * 1024.0);
    }
#else
